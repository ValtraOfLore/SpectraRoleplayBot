...
class DiscordCommand {
    ...
    async isAccessLevel(interaction) {
        return (await this.getCommandAccessLevel(interaction)) >= DiscordCommand.getUserAccessLevel(interaction);
    }
    ...
    async execute(interaction) {
        if (this.ExecutionFunction) {
            ...
            if (await this.isAccessLevel(interaction)) {
                ...
                interaction.reply(`You do not have permission to access this command`);
            }
        } else {
    ...
    static getUserAccessLevel(interaction) {
        ...
        interaction.member?.roles?.cache?.each((role) => {
            const roleAccessLevel = dataStorage.getGuildValue(guildId, `role_access_level_${role.id}`);
            if (roleAccessLevel)
